...
const MOCK_DB = {
	...
	products: [
		{
			_id: "p1",
			productName: "Mock Minimalist Watch",
			description: "A sleek, minimalist watch.",
			price: 199.99,
			category: "Accessories",
			stockQuantity: 15,
			isActive: true,
			images: [
				"https://placehold.co/400x400/png?text=Watch",
				"https://placehold.co/400x400/png?text=Watch+2",
			],
			rating: 4.5,
			features: [
		...
		{
			_id: "p2",
			productName: "Mock Wireless Earbuds",
			...
			rating: 4.8,
			features: [{ key: "Battery Life", value: "24h" }],
			specifications: { weight: "15g", connectivity: "Bluetooth 5.3" },
		},
		{
			_id: "p3",
			productName: "Mock Ergonomic Chair",
			...
			rating: 4.2,
			features: [{ key: "Adjustable", value: "Yes" }],
...
export function setupMockApi(api: AxiosInstance) {
	...
	api.defaults.adapter = async (config) => {
		...
		const responseData: any = { data: null, message: "Success", code: 200 };
		...
		try {
			...
			// 1. Auth & Users
			if (
				urlWithoutQuery.includes("/auth/me") ||
				urlWithoutQuery.includes("/user/profile")
			) {
